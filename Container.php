...
class Container //implements LoggerAwareInterface
{
    ...
    function sendFile($name, $contents)
    {
        return \Amp\call(function() use ($name, $contents) {
            ...
            yield $this->hostExec("tar cf - -C " . escapeshellarg(__DIR__) . " $fname | lxc exec {$this->name} -T --user 1000 --group 1000 -- tar xf - -C /home/codesand");
    ...
    function runGolang(string $code)
    {
        ...
        return \Amp\call(function () use ($code) {
            ...
            return $this->runCMD("lxc exec {$this->name} --user 1000 --group 1000 -T --cwd /home/codesand -n -- go run /home/codesand/$fname ; echo");
        });
    }
